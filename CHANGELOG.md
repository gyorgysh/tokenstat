...
- A tool row reads as what it did: the verb, the file, and how many lines
  moved. Small diffs open on their own, and one that is still arriving keeps
  opening as it grows, until you open or close it by hand.
- Sending an image with nothing typed works, and the image stays in the
  ...
  visible content.
- A conversation loaded to its first message says Start of chat. It used to
  read as stuck part way through history.
