...
| Interactive terminal emulator | built | built | built (xterm.js WebView over pty.*; nextOffset resume with backoff and liveness, dropped/paused banners, TerminalPalette and JetBrains Mono, selection copy, Close/Done; spawn/attach failures surface a fatal-error card with retry instead of a blank view) |
...
| SSH connect + key import | built | built | built (Use picker with password plus saved keys preselecting the host credential, generate/import; host/key/snippet rows with popup menus for connect/edit/copy/delete/favourites/run-on-connect; vault Sync/Delete in the card popup menu; live session with Done/End, snippets, vault; no session tabs or suggest palette) |
| Screen viewer (Legend) | built | built | implemented in source (MediaCodec H.264, touch input, control flip, heartbeat, reconnect, viewing notice, keyboard row, display picker, clipboard sync, PCM audio, quality selection and file transfer; connected-device acceptance still required) |
...
| App icon (three bars on dark paper) | Adaptive `mipmap/ic_launcher` + `drawable/app_icon.xml` from `store/play-icon.svg`; notification glyph is the bars, not a T; launch `windowBackground` is Theme paper in light and night; animated launch splash (core-splashscreen; bars rise staggered on theme paper) plus a monochrome themed-icon layer; no skeleton or spinner on the first frame | done in source (install icon, splash frame, and notification rendering need a physical device) |
...
| DevicesView (rows, awake dot, detail) | `DevicesScreen` host cards with awake dot, distro brand (`marks/DeviceMarks.kt` `distroBrandId` port of `distroBrandID`, pinned by `DeviceMarksTest`), spend share over `account.machineUsage`, You chip, chevron; setup and SSH cards; detail with Last sync/Last used | done |
| HomeView (greeting, totals, heatmap card, limits, lock banner) | `HomeScreen` draws the arranged sections in phone balanced order (usage, continue, machines, pinned, activity, limits) with the Apple copy; continue/pinned shelves over `HomeStores` with pin toggles; awake-hosts machines section; `Customize Home` editor with presets, layout preview strip, section glyphs, reorder, hide, reset; offline/empty/error status with getting-started card; limits sorted closest-to-full-first with severity gauges and observed dates | done |
...
| `ClientRecentPlaces` (continue shelf, 20, account scope) | `logic/HomeLogic.kt` `RecentPlaces` + `HomeStores` (SharedPreferences); account identity uses handle/account-id fallback; records folder, chat and terminal opens through `TokenstatApp.kt` | implemented in source |
...
| `ClientWorkspacesView`/`ClientHostWorkspacesView` (host cards, folders/chats/sessions, picker, customize) | `workspace/HostWorkspaces.kt` (`HostCard`, `WorkspaceFolderRow`, `WorkspaceSessionRow`, `WorkspaceChatRow`, `FolderChooserDialog`, `WorkspacesEditor` with preview, `WorkspaceLayoutStore`) + `workspace/FolderPicker.kt` (`fs.browse`/`fs.mkdir`/`workspace.add`); explicit connect/disconnect with `workspace.access.check` plus ask-and-watch, last-host auto-connect, pull-to-refresh through `TsRefresh` | done |
...
| AccountSheet (tier badge, products, sign out) | AccountDialog with the same three panes (Account / This device / Legal) over a SegmentedCapsulePicker; TierMark, notify toggle ("or a chat" copy), Sync privacy card, legal URLs, "Delete on website…" with the permanence copy, PaywallSheet | done |
| Paywall (gradient tier marks, tier-switch spring) | `billing/PaywallSheet.kt` wraps Play Billing; feats and summaries match `ClientStore` word-for-word with relay allowances as Apple captions; "Your current plan" marker; Play renew note (Google Play wording where Apple says App Store) | implemented in source (no spring; eligible Play offer intro phases are displayed, with account trial-used gating; real Play purchase/change/restore checks still required) |
...
| `SSHLibraryView` (hosts/keys/snippets, vault) | `ssh/SshRows.kt` rows with popup menus (connect/favourite/edit/delete, copy public/fingerprint, run-on-connect), `SshConnectDialog` Use picker, host/key/snippet edit dialogs, vault Sync/Delete in the card menu | done |
