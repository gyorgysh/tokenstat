    ...
    protected override void OnLaunched(LaunchActivatedEventArgs args)
    {
        ...
        body.Loaded += async (_, _) =>
        {
            ...
            try
            {
                ...
                var contextAction = new Button { Content = "Run test action" };
                var invoked = 0;
                contextAction.Click += (_, _) => invoked++;
                body.Children.Add(contextAction);
                await Task.Delay(50);
                var contextMenu = ContextMenus.Menu(contextAction);
                ContextMenus.AddButton(contextMenu, contextAction);
                ContextMenus.Invoke(contextAction);
                await Task.Delay(50);
                if (invoked != 1 || contextMenu.Items.Count != 1) throw new Exception("Context action did not share the button handler");
                contextAction.IsEnabled = false;
                ContextMenus.Invoke(contextAction);
                await Task.Delay(50);
                if (invoked != 1) throw new Exception("Context action bypassed a disabled button");
                body.Children.Remove(contextAction);
                Program.Log("PASS: context actions share button handlers and respect disabled actions");
