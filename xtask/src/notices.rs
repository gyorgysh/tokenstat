...
fn render(root_crate: &str, crates: &[Crate]) -> String {
    ...
    // No "generated by, do not edit" line. This file is written into `target/`
    // on every build and is never committed, so there is no hand-edit for that
    // warning to prevent. Its only real reader is somebody on the Open source
    // licenses screen in the app, to whom the name of our build task means
    // nothing.
    out.push_str("Licence texts are printed once each, at the end, with the packages that\n");
    out.push_str("share them.\n\n");
